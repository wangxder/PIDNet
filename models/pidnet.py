...
def get_seg_model(cfg, imgnet_pretrained):
    if 's' in cfg.MODEL.NAME:
        ...
        print("^^^^^^^^^^my own model^^^^^^^^^^")
        model = MyPIDNet(m=2, n=3, num_classes=cfg.DATASET.NUM_CLASSES, planes=32, ppm_planes=96, head_planes=128,
                         augment=True)
    else:
...
def get_pred_model(name, num_classes):
    if 's' in name:
        ...
        print("^^^^^^^^^^my own model^^^^^^^^^^")
        model = MyPIDNet(m=2, n=3, num_classes=num_classes, planes=32, ppm_planes=96, head_planes=128,
                         augment=True)
    else:
        model = PIDNet(m=3, n=4, num_classes=num_classes, planes=64, ppm_planes=112, head_planes=256,
                       augment=True)
